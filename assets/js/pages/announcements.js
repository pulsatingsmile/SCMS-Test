...
function render() {
  ...
  // Get saved IDs
  const savedIds = Storage.get(Storage.KEYS.SAVED_ANNOUNCEMENTS, []);
...
window.handleSave = (id) => {
  ...
  if (savedIds.includes(id)) {
    // Unsave
    Storage.removeItem(Storage.KEYS.SAVED_ANNOUNCEMENTS, id);
    Toast.show(`Unsaved: ${item.title}`);
  } else {
    // Save
    Storage.addItem(Storage.KEYS.SAVED_ANNOUNCEMENTS, id);
